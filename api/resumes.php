...
// Автомиграция: поле «опыт работы» в резюме (добавляем, если столбца ещё нет).
try {
    $pdo->exec("ALTER TABLE resumes ADD COLUMN experience VARCHAR(255) NULL AFTER profession");
...
elseif ($method === 'POST') {
    ...
    $sql = "INSERT INTO resumes (
                ...
                profession=VALUES(profession), experience=VALUES(experience), education_level=VALUES(education_level),
    ...
    $params = [
        ...
        $input['profession'] ?? '', $input['experience'] ?? '', $input['education_level'] ?? '', $input['education_institution'] ?? '',
